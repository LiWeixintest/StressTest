from ctypes import *
from libs.can.zlg.zlgcan import *
...
class ZUDS(object):
    def __init__(self):
        if platform.system() == "Windows":
            self.__dll = windll.LoadLibrary("./libs/can/zlg/zuds.dll")
        else:
...
#                 print("[%d]:timestamps:%d,type:CAN, %s ,id:%s, dlc:%d, eff:%d, rtr:%d, data:%s" % (i, rcv_msg[i].timestamp,
#                                                                                                    "tx" if (rcv_msg[i].frame.__pad & 0x20) else "rx",
#                                                                                                    hex(rcv_msg[i].frame.can_id),
...
#     zudslib = ZUDS()
#     zcanlib = ZCAN()
#     device_handle = zcanlib.OpenDevice(ZCAN_USBCANFD_200U, 0, 0)  # 打开设备
...
#     chn_handle = canfd_start(zcanlib, device_handle, 0)  # 初始化CAN卡，获取通道句柄，并且开启队列发送模式
#     print(hex(chn_handle))
