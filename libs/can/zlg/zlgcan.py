...
class LENTH(Structure):
    _fields_ = [("len", c_char_p)]


class ZCAN(object):
    def __init__(self):
        if platform.system() == "Windows":
            self.__dll = windll.LoadLibrary(r"./libs/can/zlg/zlgcan.dll")
        else:
    ...
    def ReceiveFD(self, chn_handle, rcv_num, wait_time=1):
        try:
    ...
    def ZCAN_GetValue(self, device, path):
        try:
            return self.__dll.ZCAN_GetValue(device, path.encode("utf-8"))
        except:
...
def canfd_start(zcanlib, device_handle, chn, aBit, dBit):
    ...
    ret = zcanlib.SetValue(ip, str(chn) + "/canfd_abit_baud_rate", str(aBit))  # 设置波特率
    ret = zcanlib.SetValue(ip, str(chn) + "/canfd_dbit_baud_rate", str(dBit))
    if ret != ZCAN_STATUS_OK:
    ...
    ret = zcanlib.SetValue(ip, str(chn) + "/filter_start", "0x0")
    if ret != ZCAN_STATUS_OK:
    ...
    ret = zcanlib.SetValue(ip, str(chn) + "/set_send_mode", "0")  # 设置启动队列发送模式，需要在startcan之后设置
    if ret == ZCAN_STATUS_OK:
        print("set_queue_send_mode success!")
    else:
        print("set queue fail")
    ret = zcanlib.SetValue(ip, str(chn) + "/set_device_tx_echo", "1")  # 发送回显：0-关闭 ，1-开启
    if ret == ZCAN_STATUS_OK:
        print("set_device_tx_echo success!")
    else:
        print("set_device_tx_echo fail")
...
if __name__ == "__main__":
    ...
    AutoCAN_B = ZCANFD_AUTO_TRANSMIT_OBJ()
    ...
    '''
    chn_handle1 = canfd_start(zcanlib, handle, 1)
    print("channel handle1:%d." %(chn_handle1))
    '''
    # lenth=LENTH()
    # lenth.len= c_char_p(zcanlib.ZCAN_GetValue(handle,"0/get_device_available_tx_count/1"))
    # len=(ord(lenth.len.decode('utf-8')))
    ...
    for i in range(transmit_num):
        msgs[i].transmit_type = 0  # normal transmit
    ...
    ret = zcanlib.ReadChannelErrInfo(chn_handle)
    if ret != 0:
        print("ReadChannelErrInfo success")
    else:
        print("ReadChannelErrInfo Fail")
    ...
    '''
    ret=zcanlib.ResetCAN(chn_handle1)
    if ret==1:
        print("ResetCAN success! ")
    '''
    # Close Device
